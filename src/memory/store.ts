...
export interface MemoryStore {
  ...
  read(): Promise<string>;
  appendDaily(entry: { did: string[]; decided: string[]; blocked: string[]; next: string }): Promise<void>;
  remember(category: string, content: string): Promise<void>;
  relevant(task: string, limit?: number): Promise<string[]>;
  saveResume(state: Omit<ResumeState, "updatedAt">): void;
  getResume(): ResumeState | undefined;
  close(): void;
}
...
export function openMemoryStore(projectRoot: string): MemoryStore {
  ...
  return {
    async ensureFile() {
      ...
      if (!existsSync(memoryPath)) await writeFile(memoryPath, MEMORY_TEMPLATE, "utf8");
    },
    async read() {
    ...
    async appendDaily(entry) {
      ...
      for (const item of [...entry.did, ...entry.decided, ...entry.blocked, entry.next]) insertMemory.run(randomUUID(), projectRoot, "session", item, "MEMORY.md", 1, new Date().toISOString());
    },
    ...
    getResume() {
      const result = readResume.get(projectRoot) as ResumeState | undefined;
      return result;
    },
    close() { db.close(); },
  };
