import type { RepositoryFile, RepositorySnapshot } from "./repository.js";
...
function scoreFile(file: RepositoryFile, taskTerms: string[]): { score: number; reason: string } {
  ...
  if (basename.includes("test") || basename.includes("spec")) score += 1;
  if (path === "package.json" || path === "tsconfig.json" || path.endsWith("/README.md")) score += 5;
  if (path === "package-lock.json" || path.endsWith("/package-lock.json") || path === "pack-plan.json") score -= 20;
  if (file.bytes > 100_000) score -= 8;
  return { score, reason: matches.length ? `path matches: ${matches.join(", ")}` : file.isSource ? "source/config file" : "non-source metadata" };
}

export async function selectContext(snapshot: RepositorySnapshot, task: string, budgetTokens: number): Promise<ContextSelection> {
  if (!Number.isInteger(budgetTokens) || budgetTokens <= 0) throw new Error("Context budget must be a positive integer.");
  ...
  const omitted: Array<{ relativePath: string; reason: string }> = [];
  let total = 0;
