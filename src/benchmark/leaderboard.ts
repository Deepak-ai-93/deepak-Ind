...
export function fixtureSetFingerprint(cases: BenchmarkCase[]): string {
...
export function createLeaderboardEntry(cases: BenchmarkCase[], results: BenchmarkResult[], now = new Date().toISOString()): LeaderboardEntry {
  if (results.length === 0) throw new Error("Cannot create a leaderboard entry without benchmark results.");
...
export function leaderboardMarkdown(entries: LeaderboardEntry[]): string {
...
export async function writeLeaderboard(outputRoot: string, entry: LeaderboardEntry): Promise<{ jsonl: string; markdown: string }> {
  await mkdir(outputRoot, { recursive: true });
  const jsonl = join(outputRoot, "leaderboard.jsonl");
  const markdown = join(outputRoot, "leaderboard.md");
  let entries: LeaderboardEntry[] = [];
  try { entries = (await readFile(jsonl, "utf8")).split(/\r?\n/).filter(Boolean).map((line) => JSON.parse(line) as LeaderboardEntry); } catch { /* first run */ }
