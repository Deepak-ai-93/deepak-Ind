...
export interface SecurityIssue { file: string; line: number; message: string; }
...
export async function scanProjectSecurity(root: string): Promise<SecurityIssue[]> {
  ...
  async function walk(directory: string): Promise<void> {
    for (const entry of await readdir(directory, { withFileTypes: true })) {
      ...
      if (content.includes("\u0000") || content.length > 1_000_000) continue;
      content.split(/\r?\n/).forEach((line, index) => {
        for (const [pattern, message] of SECRET_PATTERNS) if (pattern.test(line)) issues.push({ file: relative(root, absolute).replaceAll("\\", "/"), line: index + 1, message });
      });
    }
  }
  await walk(root);
