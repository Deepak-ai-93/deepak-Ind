...
import type { ChunkAction, TaskPlan, TaskRunEvent } from "./types.js";

export interface RunPlanOptions {
  projectRoot: string;
  approvedChunks: Set<string>;
}

export async function runTaskPlan(plan: TaskPlan, actions: Map<string, ChunkAction[]>, options: RunPlanOptions, onEvent?: (event: TaskRunEvent) => void): Promise<TaskPlan> {
  const emit = (event: TaskRunEvent) => onEvent?.(event);
  for (const chunk of plan.chunks) {
    ...
    if (!options.approvedChunks.has(chunk.id)) { chunk.status = "blocked"; throw new Error(`Chunk ${chunk.id} is blocked until approved.`); }
    ...
    try {
      for (const action of actions.get(chunk.id) ?? []) {
        if (action.type === "edit") {
          ...
          if (result.timedOut || result.exitCode !== 0) throw new Error(`Verification failed for '${action.command}': ${result.stderr || result.stdout}`);
        }
      }
      chunk.status = "passed";
