...
export type ApprovalMode = "chunk" | "command" | "never";
...
const DEFAULTS = {
  ...
  cheapModel: "",
  strongModel: "",
  routing: "auto" as "auto" | "off",
  approval: "chunk" as ApprovalMode,
...
function approvalValue(value: unknown): ApprovalMode {
  return value === "chunk" || value === "command" || value === "never" ? value : DEFAULTS.approval;
}

function routingValue(value: unknown): "auto" | "off" {
...
function configPath(projectRoot: string): string {
  ...
  if (!explicit) return join(projectRoot, ".ind", "config.json");
...
export function loadConfig(start = process.cwd()): IndConfig {
...
export function configFileLocation(projectRoot: string): string {
