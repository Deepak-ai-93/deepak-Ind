...
function install(): void {
  ...
  if (result.status !== 0) throw new Error(`OpenCode installation failed with exit code ${result.status ?? 1}.`);
}

export function launchOpenCode(args: string[]): void {
  const command = executableName();
  if (!canRun(command)) {
    install();
    if (!canRun(command)) throw new Error("OpenCode installed, but its executable is not on PATH. Restart the terminal and run 'ind opencode' again.");
  }
