import { ProviderError, type ChatEvent, type ChatMessage, type ChatRequest, type ChatTool, type ProviderAdapter, type ProviderCapabilities, type Usage } from "./types.js";
...
function toMessage(message: ChatMessage): OpenAIMessage {
...
function toTools(tools: ChatTool[] | undefined): Array<{ type: "function"; function: ChatTool }> | undefined {
...
export class OpenAICompatibleAdapter implements ProviderAdapter {
  ...
  constructor(options: OpenAICompatibleOptions) {
    if (!options.baseUrl.trim()) throw new Error("OpenAI-compatible provider requires a base URL.");
    ...
    try {
      while (true) {
        ...
        for (const line of lines) {
          ...
          if (parsed === "[DONE]") { yield { type: "done", finishReason }; return; }
