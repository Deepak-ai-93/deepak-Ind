...
export function routeTask(task, config) {
    ...
    if (config.routing === "off" || !config.cheapModel || !config.strongModel)
        return { kind, model: config.model, tier: "configured", reason: "automatic routing is disabled or tier models are not configured" };
    const cheapKinds = new Set(["summarize", "inspect"]);
    if (cheapKinds.has(kind))
        return { kind, model: config.cheapModel, tier: "cheap", reason: patterns.find(([candidate]) => candidate === kind)?.[2] ?? "low-complexity task" };
