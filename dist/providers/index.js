...
export function createConfiguredProvider() {
    ...
    if (config.provider === "google")
        return new GoogleAdapter({ apiKey: process.env.GOOGLE_GENERATIVE_AI_API_KEY ?? "" });
    if (config.provider === "openai")
        return new OpenAICompatibleAdapter({ id: "openai", baseUrl: config.baseUrl ?? "https://api.openai.com/v1", apiKey: process.env.OPENAI_API_KEY });
...
export async function localRuntimeSummary() {
    const runtimes = await discoverLocalRuntimes();
    if (runtimes.length === 0)
        return ["no local runtimes detected (probes: Ollama 11434, LM Studio 1234)"];
